...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Map<Class<?>, Object> typeToArg = new HashMap<>();
        typeToArg.put(HttpServletRequest.class, request);
        typeToArg.put(Map.class, view);

        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method iterMethod : clazz.getDeclaredMethods()) {
                if (route.getAction().equals(iterMethod.getName())) {
                    if (Arrays.stream(iterMethod
                            .getParameterTypes())
                            .allMatch(typeToArg::containsKey)) {
                        method = iterMethod;
                        break;
                    }
                }
        ...
        try {
            template = newTemplate(URLDecoder.decode(pageClass.getSimpleName() + languageAdd, StandardCharsets.UTF_8.name()) + ".ftlh");
        } catch (ServletException ignored) {
            template = newTemplate( URLDecoder.decode(pageClass.getSimpleName(), StandardCharsets.UTF_8.name()) + ".ftlh");
        }
